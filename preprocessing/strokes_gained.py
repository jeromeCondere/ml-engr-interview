...
def get_expected_shots_info(pga_file, pga_putting_file):
    ...
    expected_shots_dict['Tee'] = interp1d(tee_app_arg[['Distance', 'Tee']].dropna()['Distance'],
                                  tee_app_arg[['Distance', 'Tee']].dropna()['Tee'],
                                  kind='linear',
                                  fill_value='extrapolation')
    expected_shots_dict['Fairway'] = interp1d(tee_app_arg[['Distance', 'Fairway']].dropna()['Distance'],
                                      tee_app_arg[['Distance', 'Fairway']].dropna()['Fairway'],
                                      kind='linear',
                                      fill_value='extrapolation')
    expected_shots_dict['Rough'] = interp1d(tee_app_arg[['Distance', 'Rough']].dropna()['Distance'],
                                    tee_app_arg[['Distance', 'Rough']].dropna()['Rough'],
                                    kind='linear',
                                    fill_value='extrapolation')
    expected_shots_dict['Sand'] = interp1d(tee_app_arg[['Distance', 'Sand']].dropna()['Distance'],
                                   tee_app_arg[['Distance', 'Sand']].dropna()['Sand'],
                                   kind='linear',
                                   fill_value='extrapolation')
    expected_shots_dict['Green'] = interp1d(put['Distance'], put['Expected putts'], kind='linear')
